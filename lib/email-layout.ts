...
export function rowsTable(rows: [string, string][], rtl = false) {
  ...
  return rows.length ? `<table role="presentation" dir="${rtl ? "rtl" : "ltr"}" style="width:100%;border-collapse:collapse;font-size:14px;margin:8px 0 4px">${rows.map(([k, v]) => `<tr><th scope="row" style="text-align:${side};padding:${pad};color:#5E564D;font-weight:normal;vertical-align:top;width:38%">${esc(k)}</th><td style="padding:7px 0;vertical-align:top;text-align:${side}" dir="auto">${esc(v)}</td></tr>`).join("")}</table>` : "";
}
...
export function emailShell(title: string, inner: string, opts: { lang?: "en" | "ar"; footer?: string } = {}) {
  ...
  return `<!doctype html><html lang="${rtl ? "ar" : "en"}" dir="${rtl ? "rtl" : "ltr"}"><body style="margin:0;background:#F2F0EC;font-family:Arial,Tahoma,sans-serif;color:#2B2622">`
    ...
    + `<header style="padding:20px 28px;background:#4A443C;color:#ffffff"><img src="${esc(logo)}" width="84" height="64" alt="Sankari Holding" style="display:block;border:0;margin-bottom:10px;color:#ffffff;font-weight:bold;letter-spacing:.08em"><h1 style="margin:0;font-size:20px;line-height:1.3;text-align:${rtl ? "right" : "left"}">${esc(title)}</h1></header>`
    + `<section style="padding:26px 28px;text-align:${rtl ? "right" : "left"}">${inner}<p style="margin:26px 0 0;font-size:12px;color:#5E564D">${esc(opts.footer ?? "Sankari Holding · IT")}</p></section></main></body></html>`;
}
