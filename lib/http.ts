...
export const jsonError=(error:unknown)=>{const message=error instanceof Error?error.message:"Unexpected error";const safe=/required|invalid|version|not found|forbidden|too long/i.test(message)?message:"Request could not be completed";return NextResponse.json({error:safe},{status:/not found/i.test(safe)?404:/version/i.test(safe)?409:400});};
