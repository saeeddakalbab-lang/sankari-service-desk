...
export const authOptions:NextAuthOptions={
  ...
  events:{async signIn({user}){if(!user.email)return;const email=user.email.toLowerCase(),roles=new Set<Role>(["employee"]);if(csv(process.env.ADMIN_EMAILS).has(email))roles.add("admin");if(csv(process.env.BOARD_EMAILS).has(email))roles.add("board");if(csv(process.env.AGENT_EMAILS).has(email))roles.add("agent");if(csv(process.env.DEV_EMAILS).has(email))roles.add("dev");await import("./db").then(({query})=>query(`UPDATE users SET roles=ARRAY(SELECT DISTINCT unnest(roles || $2::text[])) WHERE id=$1`,[user.id,[...roles]]));}},
  secret:process.env.NEXTAUTH_SECRET
};
export async function currentUser():Promise<User|null>{
  if(process.env.NODE_ENV!=="production"&&process.env.AUTH_DEV_BYPASS==="true")return {id:"00000000-0000-4000-8000-000000000001",email:"dev@sankari-holding.com",name:"Development Admin",image:null,roles:["employee","agent","admin","board","dev"]};
